...
# This compares cloudentity/encoding/json to the standard golang encoding/json;
...
bench-simple: $(benchstat)
	...
	@go test -v -run '^$$' -bench /codeResponse -benchmem -benchtime 3s -cpu 1 ./json -count 8 | tee /tmp/cloudentity-encoding-json.txt
	benchstat /tmp/encoding-json.txt /tmp/cloudentity-encoding-json.txt

bench-master: $(benchstat)
	git stash
	git checkout master
	@go test -v -run '^$$' -bench /codeResponse -benchmem -benchtime 3s -cpu 1 ./json -count 8 | tee /tmp/cloudentity-encoding-json-master.txt
	git checkout -
	git stash pop
	@go test -v -run '^$$' -bench /codeResponse -benchmem -benchtime 3s -cpu 1 ./json -count 8 | tee /tmp/cloudentity-encoding-json.txt
	benchstat /tmp/cloudentity-encoding-json-master.txt /tmp/cloudentity-encoding-json.txt
